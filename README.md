...
crates/wasm/               nc2000-wasm JS bridge (M9): Dex / Battle / Searcher (stepped skuct — the
                           ...
                           info agent: observe/step/best + beliefInfo), JSON-string API, embedded dex;
                           build.sh = tuned wasm-pack build; tests-node/ = parity + bench twins vs native
web/                       Vite+Preact browser demo (M9): worker-threaded bot with ponder, baked-table
                           preview, device benchmark; bot plays blind by default (M10c) with an X-ray
                           perfect-info toggle + "bot's read" belief chip; meta pool + pair tables
                           fetched from /data/* at runtime (never bundled — the background bake extends
                           the app in place)
...
cd web && npm run build && npm run preview   # /data/* served from repo data/ in dev AND preview
