...
crates/bot/                bots: random / max-damage / open-loop DUCT MCTS (M5) + heavy playouts, static
                           ...
                           (M10a), blind imperfect-info agent (M10b); examples: arena / play / tune /
                           profile_mcts / bake_preview
crates/wasm/               nc2000-wasm JS bridge (M9): Dex / Battle / Searcher (stepped skuct — the
                           ponder substrate) / PreviewTables, JSON-string API, embedded dex; build.sh =
                           tuned wasm-pack build; tests-node/ = parity + bench twins vs native
web/                       Vite+Preact browser demo (M9): worker-threaded skuct bot with ponder, baked-
                           table preview, device benchmark; meta pool + pair tables fetched from /data/*
                           at runtime (never bundled — the background bake extends the app in place)
...
10. **M10 — imperfect info: IN PROGRESS (M10a+M10b done 2026-07-17; web wiring pending — M10c).** Belief = distribution over the M8 meta sets filtered by observations; per-iteration determinization imputes sampled hidden movesets/items onto the public state. The meta-pool restriction is exactly what makes this cheap. DV-level purity and hidden-counter purity are non-goals. Gates: blind-vs-perfect-info gap measured; no psychic tells at parity budget — both landed in M10b.
