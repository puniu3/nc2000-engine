...
6. **M6 — strength core (rollout/eval quality)**: truncated rollouts + hand-written static eval (type matchups, HP, status, boosts, PP), ε-greedy heavy rollout policy, weights tuned by self-play (SPSA/CMA-ES over the arena). This is the only axis that lifts the saturated budget curve (see the M5 ladder). Deferred M4 perf ideas get pulled in only where the post-truncation profile says they matter. Gate: ≥0.65 vs current mcts at equal wall-clock; ladder re-measured with the saturation point visibly moved.
7. **M7 — mixed strategies**: state-keyed tree (split by chance outcome, fixing open-loop aliasing) + regret matching (RM+/EXP3) at simultaneous decision nodes replacing decoupled UCB1; root play samples the average strategy instead of argmax. Whole-game CFR solving is out of scope — GTO here means per-decision-point equilibrium approximation. Gate: parity or better vs M6-DUCT at equal budget, and lower exploitability (a best-response bot tuned against the frozen argmax policy must gain less against the mixed one).
