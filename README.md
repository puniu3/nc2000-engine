...
8. **M8 — meta pool + baked tables**: curate the NC2000 meta team/set pool (human work; replaces the random fixture pool as the product distribution), then bake dev-time compute into shipped tables: **team-preview policy per matchup as an offline-solved (mixed) equilibrium** — fill the preview payoff matrix with thousands of `skuct` self-play games per cell (dev-time compute is unbounded, so cells are exact where M7's online estimates were noise), solve with M7's full-width RM+ solver (`smmcts::solve_rm_plus`), ship the resulting distribution; this is where mixed strategies actually ship, because the M7 failure mode (estimation error > true value differences) disappears and the adapting adversary (humans studying the tables) exists. Plus tuned eval weights and opening lines. In-battle search: `skuct`. Gates: baked preview ≥ online preview search at runtime budget; the mixed preview equilibrium must lose less to a counter-picking best response over the meta matrix than the argmax preview does (measurable exactly, offline). Optional if wanted: an adapting evaluation adversary (learns a frozen policy's tendencies across games — a measuring instrument, not a shipped bot, so "exploitation OUT" is untouched) to certify mixing benefits end-to-end.
