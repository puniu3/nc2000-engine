...
## 12. 対戦相手のハンドル名の扱い（オーナー裁定 2026-07-20）

**対戦相手のハンドル名は public repo に載ってよい**（オーナー裁定、2026-07-20）。
`crates/bot/examples/replay_analysis.rs:1` と `crates/bot/tests/evasion.rs:5` の
「sohehe-vs-puniu3 replay」表記はそのままでよい。

経緯: 私は commit `9f3e483` で一度これを匿名化したが、過剰修正だったため差し戻した。
commit `4a5b0f9` がリプレイHTMLと spectator ログを `.gitignore` で除外しているのは
**別の判断**であり、本裁定では変更していない。次セッションで再度匿名化しないこと。
