            ...
            if let Some(act) = self.script.get(n).cloned() {
                ...
                if let Some(c) = resolved {
                    if self.dead_filter && is_dead(b, dex, side, c, self.wake_rule) {
                        self.replaced += 1;
                        return heur_suicune(b, dex, side, choices, self.dead_prefer_ice);
                    }
                    self.scripted += 1;
                    return c;
                }
...
/// ADVERSARIAL ADDITION: swap every occurrence of one move key for another
/// inside a recorded line, leaving every other pick (Rest, Sleep Talk,
/// Struggle, switches) exactly where the human put it.
fn subst(keys: &[&'static str], from: &str, to: &'static str) -> Vec<&'static str> {
    keys.iter().map(|&k| if k == from { to } else { k }).collect()
}
...
struct ArmSpec {
    ...
    tail: Tail,
    dead_filter: bool,
    /// ADVERSARIAL ADDITION: which move replaces a provably dead scripted
    /// pick. The shipped harness hardcodes Surf, which silently folds V4 into
    /// V3a; this makes the mirror available.
    dead_prefer_ice: bool,
}
...
fn run_range(
    ...
    for t in lo..hi {
        ...
        let mut me = ArmAgent {
            ...
            dead_prefer_ice: spec.dead_prefer_ice,
    ...
    let all_arms: Vec<ArmSpec> = match case.as_str() {
        "4070-endgame" => {
            ...
            vec![
                ...
                // ---- ADVERSARIAL CONTROLS (verifier, 2026-08-20) ----
                // `no-dead-ice`: the mirror of `no-dead`. Identical dead-pick
                // filter, but the replacement (and the tail) prefer ICE BEAM.
                // `no-dead` minus `no-dead-ice` is V4 measured on the policy
                // the player is actually talking about -- one that still
                // Rests -- instead of on the never-Rest greedy pair.
                ArmSpec {
                    name: "no-dead-ice",
                    depth: script.len(),
                    script: script.clone(),
                    tail: Tail::IceMax,
                    dead_filter: true,
                    dead_prefer_ice: true,
                },
                // `surf-sub`: the recorded line with every Ice Beam PICK
                // turned into a Surf. Nothing else changes -- the Rests, the
                // Sleep Talks and the timing are the human game's. This is
                // the only arm that isolates V4 alone.
                ArmSpec {
                    name: "surf-sub",
                    depth: script.len(),
                    script: to_acts(&subst(&played, "icebeam", "surf")),
                    tail,
                    dead_filter: false,
                    dead_prefer_ice: false,
                },
                // `ice-sub`: its mirror -- every Surf pick turned into an Ice
                // Beam. surf-sub minus ice-sub brackets V4 on the real line.
                ArmSpec {
                    name: "ice-sub",
                    depth: script.len(),
                    script: to_acts(&subst(&played, "surf", "icebeam")),
                    tail,
                    dead_filter: false,
                    dead_prefer_ice: false,
                },
                // `no-dead-sub`: dead picks removed AND every Ice Beam pick
                // turned into Surf. Its gap over `no-dead` is what V4 adds
                // once V3a is already taken.
                ArmSpec {
                    name: "no-dead-sub",
                    depth: script.len(),
                    script: to_acts(&subst(&played, "icebeam", "surf")),
        ...
        "4069-lead" | "4069-trapped" => {
            ...
            if case == "4069-lead" {
                ...
                for k in ["curse", "doubleteam", "milkdrink", "return"] {
                    ...
                    v.push(ArmSpec { name, script: rep(k), depth: 64, tail, dead_filter: false, dead_prefer_ice: false });
                }
            }
            v.push(ArmSpec {
                name: "search",
