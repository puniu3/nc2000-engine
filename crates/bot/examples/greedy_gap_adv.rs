//! ADVERSARIAL fork of `greedy_gap` (round-3 verification). Adds: per-arm
//! outcome accounting (win/loss/tie/turn-cap, so a "win rate" cannot hide
//! draws), three extra fixed opponents that break the greedy/greedy mirror
//! (`random`, `eps:<p>`, `legacy`), an OPEN-information search arm (the
//! blind arm's information handicap is the obvious alternative explanation
//! for the fiber gap), a `nodead` arm (search with a provably-dead pick
//! vetoed back to the greedy pick — the class-A repair), and a dead-move
//! census per arm.
//!
...
use nc2000_bot::agent::{Agent, MaxDamageAgent, RandomAgent};
use nc2000_bot::blind::{BlindAgent, OpenAgent};
...
/// Provably-dead selection detector, deliberately GENEROUS: it ignores the
/// `foe_can_switch` gate that the shipped mask honours, so anything it flags
/// is at least as large as class A. Three rules only, all of them proofs the
/// mask already knows how to state (`smmcts::noop_reason`):
///   * a damaging, non-self-KO move the engine's own damage core scores 0;
///   * a status-inflicting move aimed at a foe that already carries a major
///     status (`pokemon.rs::set_status`, one at a time);
///   * Sleep Talk / Snore in an awake user's hands (`moveexec.rs:521`).
fn dead_move(b: &Battle, dex: &Dex, side: usize, c: SearchChoice) -> bool {
    let SearchChoice::Move(id) = c else { return false };
    let (Some(att), Some(def)) = (b.active_id(side), b.active_id(1 - side)) else {
        return false;
    };
    let ms = dex.move_static(id);
    if ms.sleep_usable && b.poke(att).status != Status::Slp {
        return true;
    }
    if ms.category != Category::Status {
        if ms.selfdestruct {
        ...
        scratch.set_log_enabled(false);
        return engine_damage(&mut scratch, dex, att, def, id) <= 0.0;
    }
    if ms.status.is_some()
        && matches!(ms.target, "normal" | "allAdjacentFoes" | "allAdjacent")
        && b.poke(def).status != Status::None
    {
        return true;
    }
    false
...
/// A max-damage attacker that plays a uniform random legal choice (switches
/// included) with probability `eps`. Breaks the greedy/greedy mirror without
/// turning the opponent into noise.
struct EpsGreedy {
    inner: GreedyAgent,
    eps: f64,
    rng: SplitMix64,
...
impl Agent for EpsGreedy {
    ...
    fn choose(
        ...
        if matches!(choices[0], SearchChoice::Team(_)) {
            return self.inner.choose(battle, dex, side, choices);
        }
...
/// The class-A repair, applied to the shipped search's OUTPUT: if the search
/// picks a provably dead move, play the greedy pick instead. An upper bound
/// on what any mask arm over these three rules could buy.
struct NoDeadAgent {
    inner: BlindAgent,
    fallback: GreedyAgent,
}

impl Agent for NoDeadAgent {
    fn name(&self) -> String {
        "search-nodead".into()
    }
    fn choose(
        ...
        let pick = self.inner.choose(battle, dex, side, choices);
        if dead_move(battle, dex, side, pick) {
            return self.fallback.choose(battle, dex, side, choices);
        }
        pick
    }
}
...
struct Arm {
    ...
    per_trial: Vec<f64>,
    turns: u64,
    /// Outcome accounting: a 0.5 from a real tie and a 0.5 from the turn cap
    /// are not the same measurement, and a mean alone cannot tell them apart.
    n_win: u64,
    n_loss: u64,
    n_tie: u64,
    n_cap: u64,
    n_err: u64,
    /// Move-turns the arm spent, and how many of them were provably dead
    /// (type-immune attack / status onto an already-statused foe / awake
    /// Sleep Talk) — class A, measured on the arm's realised play.
    move_turns: u64,
    dead_turns: u64,
}
...
fn play_arm(
    ...
    for t in 0..ctx.trials {
        ...
        let mut me: Box<dyn Agent> = match arm {
            ...
            _ => match ctx.arm_b.as_str() {
                "open" => Box::new(OpenAgent::new(
                    RmConfig { iterations: ctx.iters, ..cfg() },
                    None,
                    me_seed,
                )),
                "nodead" => Box::new(NoDeadAgent {
                    inner: BlindAgent::new(
                        RmConfig { iterations: ctx.iters, ..cfg() },
                        pool.clone(),
                        None,
                        me_seed,
                    ),
                    fallback: GreedyAgent { guard_selfko: !ctx.allow_selfko },
                }),
        ...
        let mut foe: Box<dyn Agent> = match ctx.foe.as_str() {
            ...
            "random" => Box::new(RandomAgent::new(foe_seed)),
            "legacy" => Box::new(MaxDamageAgent::new()),
            "conformant" => Box::new(MaxDamageAgent::conformant()),
            f if f.starts_with("eps:") => Box::new(EpsGreedy {
                inner: GreedyAgent { guard_selfko: !ctx.allow_selfko },
                eps: f[4..].parse().expect("--foe eps:<p>"),
                rng: SplitMix64::new(foe_seed),
            }),
        ...
        let score = loop {
            if let Some(o) = b.outcome() {
                break match (o, p.side) {
                    (Outcome::P1Win, 0) | (Outcome::P2Win, 1) => {
                        how = "win";
                        1.0
                    }
                    (Outcome::Tie, _) => {
            ...
            if let Some(c) = picks[p.side] {
                if matches!(c, SearchChoice::Move(_)) {
                    acc.move_turns += 1;
                    if dead_move(&b, dex, p.side, c) {
                        acc.dead_turns += 1;
                    }
                }
            }
...
fn main() {
    ...
    let out_path = arg_s(&args, "--out", "tmp/verdicts-4069-4070/r3/adv/greedy-gap-adv.jsonl");
    let csv_path = arg_s(&args, "--csv", "");
    let ctx = Ctx {
        arm_b: arg_s(&args, "--arm-b", "blind"),
        ...
        for _ in 0..threads.max(1) {
            scope.spawn(|| loop {
                ...
                let row = serde_json::json!({
                    ...
                    "foe": ctx.foe,
                    "iters": ctx.iters,
                    "greedy_wltc": [g.n_win, g.n_loss, g.n_tie, g.n_cap, g.n_err],
                    "search_wltc": [s.n_win, s.n_loss, s.n_tie, s.n_cap, s.n_err],
                    "greedy_dead": [g.dead_turns, g.move_turns],
                    "search_dead": [s.dead_turns, s.move_turns],
                });
    ...
    {
        let sum = |k: &str, i: usize| -> u64 {
            rows.iter().map(|(_, r, _)| r[k][i].as_u64().unwrap_or(0)).sum()
        };
        println!(
            "arm A greedy : win {} loss {} tie {} cap {} err {}   dead {}/{} move-turns",
            sum("greedy_wltc", 0), sum("greedy_wltc", 1), sum("greedy_wltc", 2),
            sum("greedy_wltc", 3), sum("greedy_wltc", 4),
            sum("greedy_dead", 0), sum("greedy_dead", 1)
        );
        println!(
            "arm B {:6} : win {} loss {} tie {} cap {} err {}   dead {}/{} move-turns",
            ctx.arm_b,
            sum("search_wltc", 0), sum("search_wltc", 1), sum("search_wltc", 2),
            sum("search_wltc", 3), sum("search_wltc", 4),
            sum("search_dead", 0), sum("search_dead", 1)
        );
    }
